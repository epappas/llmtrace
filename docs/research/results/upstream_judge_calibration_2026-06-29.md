# Upstream judge calibration — 2026-06-29
...
Free-text `reason` strings, exact rule wording, and per-call cost data live in the sidecar `upstream_judge_calibration_2026-06-29.json` (not committed via the auto-PR — kept stable here so the diff-vs-previous report no-ops on days when verdicts are unchanged). See issue #123 for context.
